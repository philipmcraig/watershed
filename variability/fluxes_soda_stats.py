...
sodalat = ncfile.variables['lat'][:]
sodalon = ncfile.variables['lon'][:]
...
lat_rad = pl.radians(sodalat[:])
lon_rad = pl.radians(sodalon[:])
...
for i in range(lat_half.shape[0]-1): # loops over 256
    ...
    for j in range(sodalon.shape[0]): # loops over 512
...
slat2 = pl.zeros([360]); slat2[:330] = sodalat; slat2[330:] = pl.linspace(-75.25,-89.75,30)

AS = pl.zeros([35,slat2.size,sodalon.size]); AM = pl.zeros_like(AS[0])
AS[:,:30,:] = 0; AS[:,30:,:] = arc_sss
AM[:30,:] = 0; AM[30:,:] = arcmask
arc_sss = AS; arcmask = AM
del AS, AM
#
#x = pl.zeros_like(sou_sss[0]); y = pl.zeros_like(x)#; z = pl.zeros_like(x)
#for i in range(slat2.size):
#    for j in range(sodalon.size):
#        r = pearsonr(atl_sss[:,i,j],-shedfluxes[1:,8])
#        x[i,j] = r[0]
#        if r[1] <= 0.1:
#            y[i,j] = r[1]
#        else:
...
slat2 = pl.flipud(slat2)
lons,lats = pl.meshgrid(sodalon,slat2)
...
#pl.title('SODA SSS and Southern Pacific $\mathbf{Q}\cdot\mathbf{\hat{n}}$ correlation',
#         fontsize=18)
#f = pl.gcf()
...
#pl.savefig(sheddir+'variability/sss_corr_maps/SODA/short/sou_sss_sop_corr.png')

F = pl.array([shedfluxes[1:,3],shedfluxes[1:,4],shedfluxes[1:,5],divQ[1:,3]])
T = ['(a) Atlantic sector $\mathbf{Q}\cdot\mathbf{\hat{n}}$',
     '(b) Indian sector $\mathbf{Q}\cdot\mathbf{\hat{n}}$',
    '(c) Pacific $\mathbf{Q}\cdot\mathbf{\hat{n}}$',
   '(d) Arctic $P-E$']
proj = ccrs.NorthPolarStereo(); ext = [180,-180,60,90]
fig,ax = pl.subplots(2,2,figsize=(12,12))
for i in range(4):
    x,y = Regression(F[i],arc_sss,slat2,sodalon)
...
#for i in range(3):
#    x,y = Regression(F[i],pac_sss,slat2,sodalon)
...
pl.subplots_adjust(top=0.95,bottom=0.15,left=0.05,right=0.95,wspace=0.21,hspace=0.18)
#pl.savefig(sheddir+'variability/sss_reg_maps/SODA/sou_sss_panel.png')