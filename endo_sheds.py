...
bounds = pl.array([[88,-26,155,33], # Lake Onega to Mongolia
                    ])
isect = pl.array([[37.57076,61.30242], # near Lake Onega
                    [107.35527,47.70771], # in Mongolia
                    ])
label = 'NAs'
name = 'North Asian'
...
m = Basemap(lat_0=60,lon_0=100.,lat_1=45,lat_2=55,width=10000000,height=5000000,
...
m.plot(isect[:,0],isect[:,1],'ro',latlon=True)
...
boundary = pl.asarray(coords)
lonlat = pl.zeros_like(boundary)
lonlat[:,0], lonlat[:,1] = m(boundary[:,0],boundary[:,1],inverse=True)
