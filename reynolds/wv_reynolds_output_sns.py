...
ncfile = Dataset(reyndir+'reyn_av_quv_DJF.nc','r')
qu1 = ncfile.variables['Vertically-integrated zonal moisture flux (full field)'][:]
qu1_mf = ncfile.variables['Vertically-integrated zonal moisture flux (mean flow)'][:]
qu1_ed = ncfile.variables['Vertically-integrated zonal moisture flux (eddies)'][:]
qv1 = ncfile.variables['Vertically-integrated meridional moisture flux (full field)'][:]
qv1_mf = ncfile.variables['Vertically-integrated meridional moisture flux (mean flow)'][:]
qv1_ed = ncfile.variables['Vertically-integrated meridional moisture flux (eddies)'][:]
ncfile.close()

ncfile = Dataset(reyndir+'reyn_av_quv_MAM.nc','r')
qu2 = ncfile.variables['Vertically-integrated zonal moisture flux (full field)'][:]
qu2_mf = ncfile.variables['Vertically-integrated zonal moisture flux (mean flow)'][:]
qu2_ed = ncfile.variables['Vertically-integrated zonal moisture flux (eddies)'][:]
qv2 = ncfile.variables['Vertically-integrated meridional moisture flux (full field)'][:]
qv2_mf = ncfile.variables['Vertically-integrated meridional moisture flux (mean flow)'][:]
qv2_ed = ncfile.variables['Vertically-integrated meridional moisture flux (eddies)'][:]
ncfile.close()

ncfile = Dataset(reyndir+'reyn_av_quv_JJA.nc','r')
qu3 = ncfile.variables['Vertically-integrated zonal moisture flux (full field)'][:]
qu3_mf = ncfile.variables['Vertically-integrated zonal moisture flux (mean flow)'][:]
qu3_ed = ncfile.variables['Vertically-integrated zonal moisture flux (eddies)'][:]
qv3 = ncfile.variables['Vertically-integrated meridional moisture flux (full field)'][:]
qv3_mf = ncfile.variables['Vertically-integrated meridional moisture flux (mean flow)'][:]
qv3_ed = ncfile.variables['Vertically-integrated meridional moisture flux (eddies)'][:]
ncfile.close()

ncfile = Dataset(reyndir+'reyn_av_quv_SON.nc','r')
qu4 = ncfile.variables['Vertically-integrated zonal moisture flux (full field)'][:]
qu4_mf = ncfile.variables['Vertically-integrated zonal moisture flux (mean flow)'][:]
qu4_ed = ncfile.variables['Vertically-integrated zonal moisture flux (eddies)'][:]
qv4 = ncfile.variables['Vertically-integrated meridional moisture flux (full field)'][:]
qv4_mf = ncfile.variables['Vertically-integrated meridional moisture flux (mean flow)'][:]
qv4_ed = ncfile.variables['Vertically-integrated meridional moisture flux (eddies)'][:]
ncfile.close()
...
u1 = [qu1,qu1_mf,qu1_ed]; v1 = [qv1,qv1_mf,qv1_ed]
u2 = [qu2,qu2_mf,qu2_ed]; v2 = [qv2,qv2_mf,qv2_ed]
u3 = [qu3,qu3_mf,qu3_ed]; v3 = [qv3,qv3_mf,qv3_ed]
u4 = [qu4,qu4_mf,qu4_ed]; v4 = [qv4,qv4_mf,qv4_ed]

###############################################################################
norm = pl.Normalize(-200,200)
levels = [-200,-150,-100,-50,-20,-10,10,20,50,100,150,200]
fig,ax = pl.subplots(3,4,figsize=(22,12))

for i in range(3):
    ax1 = pl.subplot(3,4,1+4*i,projection=proj); ax1.coastlines(); ax1.gridlines()
    cs1 = ax1.contourf(lons, lats, u1[i],transform=ccrs.PlateCarree(),levels=levels,
                      norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('DJF',fontsize=20)
    
    ax2 = pl.subplot(3,4,2+4*i,projection=proj); ax2.coastlines(); ax2.gridlines()
    cs2 = ax2.contourf(lons, lats, u2[i],transform=ccrs.PlateCarree(),levels=levels,
                      norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('MAM',fontsize=20)
    
    ax3 = pl.subplot(3,4,3+4*i,projection=proj); ax3.coastlines(); ax3.gridlines()
    cs3 = ax3.contourf(lons,lats,u3[i],transform=ccrs.PlateCarree(),levels=levels,
                       norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('JJA',fontsize=20)
    
    ax4 = pl.subplot(3,4,4+4*i,projection=proj); ax4.coastlines(); ax4.gridlines()
    cs4 = ax4.contourf(lons,lats,u4[i],transform=ccrs.PlateCarree(),levels=levels,
                       norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('SON',fontsize=20)

f = pl.gcf()
colax = f.add_axes([0.29,0.07,0.45,0.05])
...
clb.set_label('kg/m/s',fontsize=20,labelpad=0)
#pl.tight_layout()
pl.subplots_adjust(left=0.04,right=0.98,top=0.96,bottom=0.15,wspace=0.02)

ax1.annotate('$\overline{qu}$',xy=(20,990), xycoords='figure pixels',fontsize=25)
ax1.annotate('$\overline{q}\,\overline{u}$',xy=(20,640), xycoords='figure pixels',fontsize=25)
ax1.annotate('$\overline{q\\backprime u\\backprime}$',xy=(15,290), xycoords='figure pixels',fontsize=25)

#pl.savefig(reyndir+'reyn_qu_sns.png')
###############################################################################
...
for i in range(3):
    ax1 = pl.subplot(3,4,1+4*i,projection=proj); ax1.coastlines(); ax1.gridlines()
    cs1 = ax1.contourf(lons, lats, v1[i],transform=ccrs.PlateCarree(),levels=levels,
                      norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('DJF',fontsize=20)
    
    ax2 = pl.subplot(3,4,2+4*i,projection=proj); ax2.coastlines(); ax2.gridlines()
    cs2 = ax2.contourf(lons, lats, v2[i],transform=ccrs.PlateCarree(),levels=levels,
                      norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('MAM',fontsize=20)
    
    ax3 = pl.subplot(3,4,3+4*i,projection=proj); ax3.coastlines(); ax3.gridlines()
    cs3 = ax3.contourf(lons,lats,v3[i],transform=ccrs.PlateCarree(),levels=levels,
                       norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('JJA',fontsize=20)
    
    ax4 = pl.subplot(3,4,4+4*i,projection=proj); ax4.coastlines(); ax4.gridlines()
    cs4 = ax4.contourf(lons,lats,v4[i],transform=ccrs.PlateCarree(),levels=levels,
                       norm=norm,extend='both',cmap='seismic')
    if i == 0: pl.title('SON',fontsize=20)
...
ax1.annotate('$\overline{qv}$',xy=(20,990), xycoords='figure pixels',fontsize=25)
ax1.annotate('$\overline{q}\,\overline{v}$',xy=(20,640), xycoords='figure pixels',fontsize=25)
ax1.annotate('$\overline{q\\backprime v\\backprime}$',xy=(15,290), xycoords='figure pixels',fontsize=25)

#pl.savefig(reyndir+'reyn_qv_sns.png')
##############################################################################
...
amr = ShedFluxes(sheddir,'Am',lon,lat,qu4,qv4)
amr2 = ShedFluxes(sheddir,'Am',lon,lat,qu4_mf,qv4_mf)
amr3 = ShedFluxes(sheddir,'Am',lon,lat,qu4_ed,qv4_ed)

afr = ShedFluxes(sheddir,'AfMe',lon,lat,qu4,qv4)
afr2 = ShedFluxes(sheddir,'AfMe',lon,lat,qu4_mf,qv4_mf)
afr3 = ShedFluxes(sheddir,'AfMe',lon,lat,qu4_ed,qv4_ed)

eaa = ShedFluxes(sheddir,'EAA',lon,lat,qu4,qv4)
eaa2 = ShedFluxes(sheddir,'EAA',lon,lat,qu4_mf,qv4_mf)
eaa3 = ShedFluxes(sheddir,'EAA',lon,lat,qu4_ed,qv4_ed)

ara = ShedFluxes(sheddir,'ArA',lon,lat,qu4,qv4)
ara2 = ShedFluxes(sheddir,'ArA',lon,lat,qu4_mf,qv4_mf)
ara3 = ShedFluxes(sheddir,'ArA',lon,lat,qu4_ed,qv4_ed)

ari = ShedFluxes(sheddir,'ArI',lon,lat,qu4,qv4)
ari2 = ShedFluxes(sheddir,'ArI',lon,lat,qu4_mf,qv4_mf)
ari3 = ShedFluxes(sheddir,'ArI',lon,lat,qu4_ed,qv4_ed)

arp = ShedFluxes(sheddir,'ArP',lon,lat,qu4,qv4)
arp2 = ShedFluxes(sheddir,'ArP',lon,lat,qu4_mf,qv4_mf)
arp3 = ShedFluxes(sheddir,'ArP',lon,lat,qu4_ed,qv4_ed)

soa = ShedFluxes(sheddir,'SOA',lon,lat,qu4,qv4)
soa2 = ShedFluxes(sheddir,'SOA',lon,lat,qu4_mf,qv4_mf)
soa3 = ShedFluxes(sheddir,'SOA',lon,lat,qu4_ed,qv4_ed)

soi = ShedFluxes(sheddir,'SOI',lon,lat,qu4,qv4)
soi2 = ShedFluxes(sheddir,'SOI',lon,lat,qu4_mf,qv4_mf)
soi3 = ShedFluxes(sheddir,'SOI',lon,lat,qu4_ed,qv4_ed)

sop = ShedFluxes(sheddir,'SOP',lon,lat,qu4,qv4)
sop2 = ShedFluxes(sheddir,'SOP',lon,lat,qu4_mf,qv4_mf)
sop3 = ShedFluxes(sheddir,'SOP',lon,lat,qu4_ed,qv4_ed)
...
#f = open(reyndir+'reyn_fluxes_SON.csv','w')
...
#
#pl.tight_layout()
#pl.savefig(reyndir+'reyn_profs_7914.png')