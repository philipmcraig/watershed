...
#bounds = pl.array([110,-39,155,-10]) # Austalian divide
bounds = pl.array([67,-55,360-156,10])
isect = pl.array([[144.97,-20.48], # Australian/Southern/Pacific intersection
                    [116.77,-35.00], # Southern/Australian intersection (Indian)
                    [149.57,-35.00] # Southern/Australian intersection (Pacific)
                    ])
label = 'Oz'
name = 'Australian'
...
oz_shp = m.readshapefile(sheddir+"Oz_sheds/oz_sheds","oz",linewidth=0.5,color='m')
m.plot(isect[0,0],isect[0,1],'ro')
m.plot(isect[1,0],isect[1,1],'ro'); m.plot(isect[2,0],isect[2,1],'ro')
m.drawparallels([-35])
pl.tight_layout()
...
#if shed == 1:
lonlat[0] = isect[1]; lonlat[-1] = isect[2]; lonlat[11] = isect[0]
#else:
