...
bounds = pl.array([[-62,-45,22,-15], # South America to Africa
                    [15,-45,120,-15], # Africa to Australia
                    [145,-45,360-65,-15], # Australia to South America
                    ])
isect = pl.array([[20.22,-35.00], # Southern Ocean, African divide
                    [116.77,-35.00], # Southern Ocean, Australian (west) divide
                    [149.57,-35.00], # Southern Ocean, Australian (east) divide
                    [360-70.37,-35.00], # Southern Ocean, La Plata (west) basin
                    [-55.00,-35.00], # Southern Ocean, La Plata (east) basin
                    ])
label = ['SOA','SOI','SOP']
name = ['Atlantic','Indian','Pacific']
# select which map to use:
shed = 2 # shed corresponds to index of 0-axis in bounds array
print name[shed] + ' Watershed'

if shed in range(0,2):
    m = Basemap(llcrnrlon=bounds[shed,0],llcrnrlat=bounds[shed,1],urcrnrlon=bounds[shed,2],
                urcrnrlat=bounds[shed,3],lat_ts=20.,resolution='l',projection='mill',
                suppress_ticks=True)
elif shed == 2:
    m = Basemap(llcrnrlon=bounds[shed,0],llcrnrlat=bounds[shed,1],urcrnrlon=bounds[shed,2],
                urcrnrlat=bounds[shed,3],lon_0=220.,
                o_lat_p=90.,o_lon_p=180.,resolution='l',projection='rotpole')
...
if shed == 0:
    sa_shp = m.readshapefile("sa_bas_30s_beta","sa",linewidth=0.5) # South America
    af_shp = m.readshapefile("af_bas_30s_beta","af",linewidth=0.5) # Africa
    m.plot(isect[0,0],isect[0,1],'ro',latlon=True); m.plot(isect[-1,0],isect[-1,1],'ro',latlon=True)
elif shed == 1.:
    af_shp = m.readshapefile("af_bas_30s_beta","af",linewidth=0.5) # Africa
    ...
    sa_shp = m.readshapefile("sa_bas_30s_beta","sa",linewidth=0.5) # South America
    au_shp = m.readshapefile("au_bas_30s_beta","au",linewidth=0.5) # Australasia
    m.plot(isect[3,0],isect[3,1],'ro',latlon=True); m.plot(isect[2,0],isect[2,1],'ro',latlon=True)
...
lonlat = pl.zeros([2,2])
if shed == 0.:
    lonlat[0] = isect[-1]; lonlat[1] = isect[0]
if shed == 1.:
    lonlat[0] = isect[0]; lonlat[1] = isect[1]
if shed == 2.:
    lonlat[0] = isect[2]; lonlat[1] = isect[-2]
#lonlat = pl.asarray(coords)#lonlat = MapCoords(coords,m)
#lonlat[0] = isect[shed]; lonlat[-1] = isect[shed+3]
#print lonlat

# Write clicked points to a file:
f = open(sheddir+'shed_defs/'+label[shed]+'_clicks.txt','w')
f.write('Longitude, Latitude co-ordinates of the end points of the ' + name[shed]
        + ' sector of the line defining the Southern Ocean catchment. '
...
A = pl.zeros_like(lonlat); A[:,0], A[:,1] = m(lonlat[:,0],lonlat[:,1])
m.plot(A[:,0],A[:,1],'b') # add line segments
m.plot(A[:,0],A[:,1],'ro') # add markers
