...
v_mn = pl.mean(v,axis=(0,1))#; u_pt = u - u_mn
del v
...
QV_prod = pl.zeros([lat.size,lon.size])
...
for i in range(lat.size):
    for j in range(lon.size):
        QV_prod[i,j] = VertInt(q_mn[:,i,j]*v_mn[:,i,j],sp_mn[i,j],pres,pres)
...
QV_in = newnc.createVariable('Vertically-integrated meridional moisture flux (mean flow)',
                                             pl.float64,('lat','lon'))
QV_in.units = 'kg/m/s'
QV_in.standard_name = 'meridional moisture flux (mean flow)'
QV_in[:,:] = QV_prod
