...
bounds = pl.array([[-127,-17,-54,55], # Americas, Atlantic/Pacific divide
                    [8,-36,44,42], # Africa/Middle East, Atlantic/Indian divide
                    [88,-26,155,35], # East Asia/Australia, Indian/Pacific divide
                    ])
isect = pl.array([[-117.45,52.03], # Arctic/American intersection, the "Snow Dome"
                    [41.98,40.08], # Arctic/Middle Eastern intersection
                    [92.18,32.71], # Arctic/East Asian intersection
                    [-71.03,-15.32], # American/Southern intersection
                    [20.22,-35.00], # African/Southern intersection
                    [144.97,-20.48], # Australian/Southern intersection
                    ])
label = ['Am','AfMe','EAA']
name = ['American','African and Middle Eastern','East Asian and Australian']
...
if shed == 0:
    na_shp = m.readshapefile("na_bas_30s_beta","na",linewidth=0.5) # North America
    ca_shp = m.readshapefile("ca_bas_30s_beta_ud","ca",linewidth=0.5) # Central America
    sa_shp = m.readshapefile("sa_bas_30s_beta","sa",linewidth=0.5) # South America
    m.plot(isect[0,0],isect[0,1],'ro'); m.plot(isect[3,0],isect[3,1],'ro')
elif shed == 1.:
    ...
    m.plot(isect[1,0],isect[1,1],'ro'); m.plot(isect[4,0],isect[4,1],'ro')
elif shed == 2.:
    ...
    m.plot(isect[2,0],isect[2,1],'ro'); m.plot(isect[5,0],isect[5,1],'ro')
