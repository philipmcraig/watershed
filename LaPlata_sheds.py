...
#bounds = pl.array([-79,-40,-29,-7]) # La Plata basin, Southern/Atlantic divide
bounds = pl.array([-101,-57,-30,15])
isect = pl.array([[-71.03,-15.32], # American/Southern/Pacific intersection
                    [-70.37,-35.00], # Southern/La Plata intersection (Pacific)
                    [-55.00,-35.00] # Southern/La Plata intersection (Atlantic)
                    ])

label = 'LP'
name = 'La Plata basin'
# select which map to use:
#shed = 1 # shed corresponds to index of 0-axis in isect array, 1 = Pacific, 2 = Atlantic
print 'La Plata Watershed'
...
#if shed == 1.:
sa_shp = m.readshapefile("sa_bas_30s_beta","sa",linewidth=0.5) # South America
m.plot(isect[:,0],isect[:,1],'ro')#; m.plot(isect[shed,0],isect[shed,1],'ro')
m.drawparallels([-35])
#elif shed == 2.:
#    sa_shp = m.readshapefile("sa_bas_30s_beta","sa",linewidth=0.5) # South America
#    m.plot(isect[0,0],isect[0,1],'ro'); m.plot(isect[shed,0],isect[shed,1],'ro')
#    m.drawparallels([-35])
pl.tight_layout()


#-----------------------CLICK ALONG CONTINENTAL DIVIDES-----------------------
...
#if shed == 1:
lonlat[0] = isect[1]; lonlat[-1] = isect[-1]; lonlat[2] = isect[0]
#else:
