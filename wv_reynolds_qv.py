...
qv = q*v
QV = pl.zeros([filenames1.shape[0],filenames1.shape[1],lat.size,lon.size])

#u_mn = pl.mean(u,axis=(0,1)); u_pt = u - u_mn
#del u, u_mn
...
#QU_pert = pl.zeros([lat.size,lon.size])
    ...
    for month in range(filenames2.shape[1]):
        for i in range(lat.size):
            for j in range(lon.size):
                QV[year,month,i,j] = VertInt(qv[year,month,:,i,j],surfp[year,month,i,j],pres[:],pres[:])
        #QU_prod[i,j] = VertInt(q_mn[:,i,j]*u_mn[:,i,j],sp_mn[i,j],pres,pres)
        #QU_pert[i,j] = VertInt(pl.mean(q_pt[:,:,:,i,j]*u_pt[:,:,:,i,j],axis=(0,1)),sp_mn[i,j],pres,pres)
        #    for month in range(filenames2.shape[1]):
        #        

QV_mn = pl.mean(QV,axis=(0,1))

# Save to netCDF

newnc = Dataset(sheddir+'reynolds/reyn_av_quv_7914.nc',mode='a')#,format='NETCDF4')
